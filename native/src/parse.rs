...
//     c.run(|| match result {
//         Ok(program) => Ok(cx
//             .string(serde_json::to_string(&program).expect("failed to
// serialize Program"))             .upcast()),
...
//     let options: ParseOptions = neon_serde::from_value(&mut cx,
// options_arg)?;     let callback = cx.argument::<JsFunction>(2)?;
//
//     let this = cx.this();
//     {
//         let guard = cx.lock();
...
// }
//
// pub fn parse_file_sync(mut cx: MethodContext<JsCompiler>) ->
// JsResult<JsValue> {     let c;
//     let this = cx.this();
//     {
//         let guard = cx.lock();
//         let compiler = this.borrow(&guard);
