...
// Program");                 // TODO: Source map
//                 self.c.process_js(program, &self.options)
//             }
//
//             Input::File(ref path) => {
//                 let fm = self.c.cm.load_file(path).context("failed to read
// module")?;                 self.c.process_js_file(fm, &self.options)
//             }
//
//             Input::Source(ref s) => self.c.process_js_file(s.clone(),
// &self.options),         })
//     }
//
//     fn complete(
